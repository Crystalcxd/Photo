...
@property (nonatomic, strong) CoverView *coverView;//顶部view
...
@property (nonatomic, strong) NSMutableDictionary *takePhotoDic;
...
- (void)viewDidLoad
{
    ...
    self.takePhotoDic = [[NSMutableDictionary alloc] init];
    ...
    if (CGRectEqualToRect(_previewRect, CGRectZero)) {
        self.previewRect = CGRectMake(0, CAMERA_TOPVIEW_HEIGHT, SC_DEVICE_SIZE.width, SC_DEVICE_SIZE.height - CAMERA_TOPVIEW_HEIGHT - CAMERA_BOTTOMVIEW_HEIGHT);
//        self.previewRect = CGRectMake(0, CAMERA_TOPVIEW_HEIGHT, SC_DEVICE_SIZE.width, SC_DEVICE_SIZE.width);
    }
...
//拍完照后的遮罩
- (void)addCameraCover {
    UIView *upView = [[UIView alloc] initWithFrame:CGRectMake(0, CAMERA_TOPVIEW_HEIGHT, SC_DEVICE_SIZE.width, 0)];
    upView.backgroundColor = [UIColor blackColor];
    [self.view addSubview:upView];
    self.doneCameraUpView = upView;
    
    UIView *downView = [[UIView alloc] initWithFrame:CGRectMake(0, _bottomContainerView.frame.origin.y, SC_DEVICE_SIZE.width, 0)];
...
- (void)showCameraCover:(BOOL)toShow {
    ...
    [UIView animateWithDuration:0.38f animations:^{
        ...
        upFrame.size.height = (toShow ? (SC_DEVICE_SIZE.height - CAMERA_TOPVIEW_HEIGHT - CAMERA_BOTTOMVIEW_HEIGHT) / 2  : 0);
        _doneCameraUpView.frame = upFrame;
        
        CGRect downFrame = _doneCameraDownView.frame;
        downFrame.origin.y = (toShow ? (SC_DEVICE_SIZE.height - CAMERA_TOPVIEW_HEIGHT - CAMERA_BOTTOMVIEW_HEIGHT) / 2 + CAMERA_TOPVIEW_HEIGHT : _bottomContainerView.frame.origin.y);
        downFrame.size.height = (toShow ? (SC_DEVICE_SIZE.height - CAMERA_TOPVIEW_HEIGHT - CAMERA_BOTTOMVIEW_HEIGHT) / 2 : 0);
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    TouchHelper *helper = [[TouchHelper alloc] initWithIndex:0 Frame:self.coverView.frame];
    
    NSInteger touchIndex = [helper touchViewIndexWithPoint:touchPoint];
    
    NSLog(@"touch area index %ld",touchIndex);

    [self removePhotoAtIndex:touchIndex];
...
{
    UIImageView *imageview = [self.view viewWithTag:2000 + index];
    
    if (imageview) {
        [imageview removeFromSuperview];
        [self.takePhotoDic removeObjectForKey:@(index)];
    }
}
...
//            [SCCommon saveImageToPhotoAlbum:stillImage];//存至本机
        ...
        CGFloat width = stillImage.size.width;
        CGFloat height = stillImage.size.height;
        ...
        NSInteger index = [self checkCurrentPhotoIndex];
        
        switch (index) {
            case 0:
                CGContextMoveToPoint(gc, 25, 35);
                CGContextAddLineToPoint(gc, width - 25, 35);
                CGContextAddLineToPoint(gc, width - 25, height/2 - 17);
                CGContextAddLineToPoint(gc, 25, height/2 - 17);
                CGContextClosePath(gc);
                CGContextClip(gc);
                break;
            case 1:
                CGContextMoveToPoint(gc, 25, height/2 + 17);
                CGContextAddLineToPoint(gc, width - 25, height/2 + 17);
                CGContextAddLineToPoint(gc, width - 25, height - 35);
                CGContextAddLineToPoint(gc, 25, height - 35);
                CGContextClosePath(gc);
                CGContextClip(gc);
                break;
        ...
        CGContextDrawImage(gc, CGRectMake(0, 0, width, height), [stillImage CGImage]);
        ...
        imageView = [[UIImageView alloc] initWithFrame:self.previewRect];
        imageView.image = destImg;
        ...
        if (index + 1 == self.picCount) {
            ...
            UIImage *image = [self mixImage];
            
            if (image) {
                con.postImage = image;
            }else{
                con.postImage = imageView.image;
            }
...
- (NSInteger)checkCurrentPhotoIndex
{
    NSInteger index = 0;
    
    while ([self.takePhotoDic objectForKey:@(index)]) {
        index ++;
    }
...
- (UIImage *)mixImage
{
    ...
    for (int i = 0; i < self.picCount; i ++) {
        UIImageView *imageview = [self.view viewWithTag:2000 + i];
        if (i == 0) {
            if (imageview) {
                image = imageview.image;
            }
        }else{
            UIImage *finalImg = [self addImageview:imageview.image toImage:image type:self.picCount];
            image = finalImg;
        }
    }
...
- (UIImage *)addImageview:(UIImage *)image2 toImage:(UIImage *)image1 type:(NSInteger)type{
